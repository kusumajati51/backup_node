<template>
    <v-row>
        <v-col cols="12" lg="3" sm="6" v-for="(item, i) in formattedCart" :key="i">
...
onMounted(() => {
    const size = computed(() => useCartStore().list)
    // console.log(size.value.length)  
    // productStore.fetchAll(100,1)
})
...
async function order() {
    ...
    console.log(order)
    await api.post('order', order).then(async (res) => {
